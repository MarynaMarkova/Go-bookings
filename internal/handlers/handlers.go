...
import (
	...
	"github.com/MarynaMarkova/Go-bookings/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	htmlMessage := fmt.Sprintf(`
		...
		Dear %s, <br>
		This is to confirm reservation from %s to %s.
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		// Get the block map from the session. Loop throught entire map, if we have an entry in the map, that does not exist in our posted data, and if the restriction id > 0, then it is a block we need to remove.
